...
public class MyWidgetProvider extends AppWidgetProvider {
    ...
    public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds) {
        ...
        for (int i=0; i<N; i++) {
			...
			if( sharedPreferences.getBoolean(SpinnnyCameraActivity.getShowWhenLockedPreferenceKey(), true) ) {
		    	if( MyDebug.LOG )
	...
	public void onReceive(Context context, Intent intent) {
	    ...
	    if (intent.getAction().equals("net.sourceforge.opencamera.LAUNCH_OPEN_CAMERA")) {
	    	if( MyDebug.LOG )
	    		Log.d(TAG, "Launching SpinnnyCameraActivity");
	        final Intent activity = new Intent(context, SpinnnyCameraActivity.class);
